...
export default {
    ...
    vorticity: [v, `${p} ${s}
        ...
        void main() {
            ...
            vec2 T = vUv + vec2(0.0, texelSize.y);
            vec2 B = vUv - vec2(0.0, texelSize.y);
            vec2 f = normalize(vec2(abs(texture2D(uCurl, T).x) - abs(texture2D(uCurl, B).x), abs(texture2D(uCurl, R).x) - abs(texture2D(uCurl, L).x)) + 0.0001) * curlStrength * texture2D(uCurl, vUv).x;
